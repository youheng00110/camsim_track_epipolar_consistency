...
def main() -> None:
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Resume from an existing records.rankXXX.jsonl",
    )
    ...
    try:
        ...
        if args.resume and rank_output.is_file():
            with rank_output.open("r", encoding="utf-8") as reader:
            ...
            if completed_records > len(items):
                raise RuntimeError(
                    "Resume record count exceeds source item count: "
                    f"records={completed_records}, items={len(items)}"
                )

            print(
                "[RESUME] "
                f"rank={rank} "
                f"completed={completed_records} "
                f"total={len(items)} "
                f"remaining={len(items) - completed_records}",
                flush=True,
            )

            items = items[completed_records:]

        if not items:
            print(
                f"[RESUME] rank={rank}: no remaining frames",
                flush=True,
            )

        dataset = FrameSourceDataset(items) if items else None
        ...
        with rank_output.open(output_mode, encoding="utf-8") as writer:
            progress = tqdm(
                loader if loader is not None else [],
                desc=f"rank {rank} eval",
                disable=rank != 0,
                initial=completed_records if args.resume else 0,
                total=(
