...
DEBUG_TIMESTAMPS_MAX_BATCHES = 20
DEBUG_EXPECTED_REF_COUNT = 3
DEBUG_TIMESTAMPS_SEEN = 0
############ ck func #########################
    ...
    def try_make_input_for_prediction(
        ...
        if frame_prediction_style == "diffusion_forcing":
            ...
            if isinstance(reference_latent_count, int):
                reference_latent_count_tensor = torch.full(
                    (batch_size, 1, 1),
            ...
            reference_latent_count_tensor = reference_latent_count_tensor.to(latents.device)
            reference_latent_count_tensor = torch.clamp(
                reference_latent_count_tensor, 0, sequence_length
            )
            ...
            reference_frame_indicator = torch.logical_and(
                torch.logical_not(disable_temporal).to(latents.device).expand(
        ...
        '''
        global DEBUG_TIMESTAMPS_SEEN

        if DEBUG_TIMESTAMPS and DEBUG_TIMESTAMPS_SEEN < DEBUG_TIMESTAMPS_MAX_BATCHES:
            is_rank0 = (
            ...
            if is_rank0:
                DEBUG_TIMESTAMPS_SEEN += 1

                ts_before = timesteps.detach().float().cpu()
                ts_after = made_timesteps.detach().float().cpu()
                ref_cpu = reference_frame_indicator.detach().cpu().to(torch.int32)

                if ts_before.ndim == 1:
                    ts_before = ts_before.view(1, -1, 1)
                elif ts_before.ndim == 2:
                    ts_before = ts_before.unsqueeze(-1)

                if ts_after.ndim == 1:
                    ts_after = ts_after.view(1, -1, 1)
                elif ts_after.ndim == 2:
                    ts_after = ts_after.unsqueeze(-1)

                if ref_cpu.ndim == 1:
                    ref_cpu = ref_cpu.view(1, -1, 1)
                elif ref_cpu.ndim == 2:
                    ref_cpu = ref_cpu.unsqueeze(-1)

                print("\n" + "=" * 120, flush=True)
                print(
                    f"[TIMESTEP_DEBUG] dump_idx={DEBUG_TIMESTAMPS_SEEN} "
                    f"frame_prediction_style={frame_prediction_style} "
                    f"reference_latent_count={reference_latent_count}",
                    flush=True
                )
                print(
                    f"[TIMESTEP_DEBUG] timesteps.shape(before)={tuple(ts_before.shape)} "
                    f"timesteps.shape(after)={tuple(ts_after.shape)}",
                    flush=True
                )
                print(
                    f"[TIMESTEP_DEBUG] reference_frame_indicator.shape={tuple(ref_cpu.shape)}",
                    flush=True
                )

                if additional_condition is not None and "disable_temporal" in additional_condition:
                    print(
                        f"[TIMESTEP_DEBUG] disable_temporal="
                        f"{additional_condition['disable_temporal'].detach().cpu().view(ts_before.shape[0], -1).tolist()}",
                        flush=True
                    )

                for b in range(ts_before.shape[0]):
                    print("-" * 120, flush=True)
                    print(f"[TIMESTEP_DEBUG] sample_in_batch={b}", flush=True)

                    for t in range(ts_before.shape[1]):
                        before_vals = ts_before[b, t].reshape(-1)
                        after_vals = ts_after[b, t].reshape(-1)
                        ref_vals = ref_cpu[b, t].reshape(-1)

                        print(
                            f"[TIMESTEP_DEBUG] frame={t:02d} "
                            f"ref_by_view={ref_vals.tolist()} "
                            f"ts_before={before_vals.tolist()} "
                            f"ts_after={after_vals.tolist()}",
                            flush=True
                        )

                    print(
                        f"[TIMESTEP_DEBUG] prefix_before(first 3 frames)="
                        f"{ts_before[b, :min(3, ts_before.shape[1]), 0].tolist()}",
                        flush=True
                    )
                    print(
                        f"[TIMESTEP_DEBUG] prefix_after(first 3 frames)="
                        f"{ts_after[b, :min(3, ts_after.shape[1]), 0].tolist()}",
                        flush=True
                    )
