...
import numpy as np
from dwm.models.crossview_temporal_dit_PLUCKER import get_rays
############ ck func #########################
def write_ply_xyzrgb(path, xyz, rgb):
    xyz = xyz.reshape(-1, 3).detach().cpu().numpy()
    rgb = rgb.reshape(-1, 3).detach().cpu().numpy().astype(np.uint8)
...
class CrossviewTemporalSD():
    ...
    def preview_pipeline(
        ...
        if "sequence_length_per_iteration" in self.inference_config:
            ...
            if self.should_save and not hasattr(self, "_saved_ray_overlay"):
                debug_dir = os.path.join(output_path, "ray_debug")
                os.makedirs(debug_dir, exist_ok=True)

                image_h = batch["vae_images"].shape[-2]
                image_w = batch["vae_images"].shape[-1]

                latent_h = image_h // (2 ** (len(self.vae.config.down_block_types) - 1))
                latent_w = image_w // (2 ** (len(self.vae.config.down_block_types) - 1))

                patch_h = latent_h // self.model.config.patch_size
                patch_w = latent_w // self.model.config.patch_size
                ...
                k_patch = camera_intrinsics_norm.clone()
                k_patch[..., 0, 0] = k_patch[..., 0, 0] * patch_w
                k_patch[..., 1, 1] = k_patch[..., 1, 1] * patch_h
                k_patch[..., 0, 2] = k_patch[..., 0, 2] * patch_w
                k_patch[..., 1, 2] = k_patch[..., 1, 2] * patch_h
                ...
                img_tensor = batch["vae_images"][b, t].detach().cpu()
                ...
                depths = torch.tensor([5.0, 10.0, 20.0, 40.0], dtype=rays_d.dtype)

                for v in range(img_tensor.shape[0]):
                    img_pil = torchvision.transforms.functional.to_pil_image(
                        img_tensor[v].clamp(0, 1)
                    )
                    draw = ImageDraw.Draw(img_pil)

                    for gy in range(0, patch_h, step):
                        for gx in range(0, patch_w, step):
                            px = (gx + 0.5) * image_w / patch_w
                            py = (gy + 0.5) * image_h / patch_h

                            d = rays_d[v, gy, gx].detach().cpu()
                            color = ((d + 1.0) * 0.5 * 255.0).clamp(0, 255).to(torch.uint8)
                    ...
                    img_pil.save(os.path.join(debug_dir, f"ray_overlay_b0_t0_view{v}.png"))

                    d_sub = rays_d[v, ::step, ::step].detach().cpu()
                    o = rays_o[v].detach().cpu().view(1, 1, 1, 3)
                    pts = o + depths.view(1, 1, -1, 1) * d_sub.unsqueeze(2)

                    img_hw = (
                        img_tensor[v].permute(1, 2, 0).clamp(0, 1) * 255.0
                    ).to(torch.uint8)

                    y_idx = (
                        (torch.arange(0, patch_h, step, dtype=torch.float32) + 0.5)
                        * image_h / patch_h
                    ).long().clamp(0, image_h - 1)

                    x_idx = (
                        (torch.arange(0, patch_w, step, dtype=torch.float32) + 0.5)
                        * image_w / patch_w
                    ).long().clamp(0, image_w - 1)

                    colors = img_hw[y_idx[:, None], x_idx[None, :]]
                    colors = colors.unsqueeze(2).expand(-1, -1, depths.numel(), -1)

                    write_ply_xyzrgb(
                        os.path.join(debug_dir, f"ray_frustum_b0_t0_view{v}.ply"),
                        pts,
                        colors
                    )

                    all_pts.append(pts.reshape(-1, 3))
                    all_rgb.append(colors.reshape(-1, 3))

                write_ply_xyzrgb(
                    os.path.join(debug_dir, "ray_frustum_b0_t0_all_views.ply"),
                    torch.cat(all_pts, dim=0),
                    torch.cat(all_rgb, dim=0)
                )

                self._saved_ray_overlay = True
        else:
            latent_shape = (
