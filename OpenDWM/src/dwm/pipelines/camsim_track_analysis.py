...
import dwm.analysis.entity_reactor as entity_reactor
import dwm.common
...
class CrossviewTemporalSD():
    ...
    def get_conditions(
        ...
        if debug_pv_layout_condition:
            ...
            if is_rank0:
                print(
                    ...
                    f"has_instance_flow_images={'instance_flow_images' in batch}",
                    flush=True,
                )
        ...
        if use_pv_layout_condition:
            ...
            if "instance_flow_images" in batch:
                if condition_on_all_frames:
                    instance_flow_images = batch["instance_flow_images"].to(device)
                else:
                    instance_flow_images = batch["instance_flow_images"][:, :1].to(device)
                ...
                if do_classifier_free_guidance:
                    instance_flow_images = torch.cat([
                        torch.ones_like(instance_flow_images) * uncondition_image_color,
                        instance_flow_images
                    ])

                condition_image_list.append(instance_flow_images)
            ...
            if debug_pv_layout_condition:
                ...
                if is_rank0:
                    print(
                        ...
                        "Skip 3dbox_images, hdmap_images and instance_flow_images. "
    ...
    def __init__(
        ...
        if resume_from is not None:
            ...
            if (
                ...
                if len(expanded_condition_keys) > 0:
                    print(
                        "expanded condition adapter input weights: {}".format(
                            expanded_condition_keys
                        )
                    )
        ...
        if torch.distributed.is_initialized():
            if self.distribution_framework == "ddp":
                ...
                ddp_wrapper_settings = dict(self.common_config["ddp_wrapper_settings"])
                ddp_wrapper_settings["auto_wrap_policy"] = ModuleWrapPolicy({
    ...
    def inference_pipeline(
        ...
        for i in range(start_timestep, stop_timestep):
            ...
            with self.get_autocast_context():
                model_output, _, _ = dwm.analysis.entity_reactor.entity_reactor_forward(
                    self,
                    latent_model_input,
                    timesteps_input,
                    model_conditions,
