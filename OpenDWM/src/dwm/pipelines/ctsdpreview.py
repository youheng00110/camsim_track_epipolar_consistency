...
class CrossviewTemporalSD():
    ...
    def _export_box_eval_artifacts(
        ...
        export_box_image = bool(
            self.inference_config.get("eval_frame_export_box_image", False)
        )
        export_box_params = bool(
            self.inference_config.get("eval_frame_export_box_params", False)
        )
        ...
        if "3dbox_images" not in batch:
            raise KeyError(
                ...
                "Enable validation dataset `_3dbox_image_settings`."
            )

        if export_box_params and "3dbox_records" not in batch:
            raise KeyError(
                "`eval_frame_export_box_params=true` requires "
                "batch['3dbox_records']. Enable validation dataset "
                "`enable_3dbox_records=true`."
            )

        if export_box_params and "box_reference_to_camera" not in batch:
            raise KeyError(
                "`eval_frame_export_box_params=true` requires "
                "batch['box_reference_to_camera']."
            )
        ...
        box_export_config["eval_frame_export_box_image"] = export_box_image
        box_export_config["eval_frame_export_box_params"] = export_box_params
        ...
        dwm.utils.preview.save_ctsd_eval_frames_for_preview(
            ...
            image_quality=self.inference_config.get(
                ...
                self.inference_config.get("eval_frame_image_quality", 95),
            ),
        ...
        if save_this_rank:
            ...
            if eval_frame_export_path is not None:
                ...
                self._export_box_eval_artifacts(
                    batch,
                    self.inference_config.get("eval_frame_export_path", None),
                    dist_on,
                    rank,
                    all_rank_preview,
                )
    ...
    def evaluate_pipeline(
        ...
        for i, batch in enumerate(validation_dataloader):
            ...
            # Optional evaluation export. This makes src/dwm/evaluate.py
            # save generated samples with the same frame/manifest structure
            # used by the preview eval-frame exporter.
            eval_frame_export_path = self.inference_config.get(
                "eval_frame_export_path", None
            )
            if eval_frame_export_path is not None:
                ...
                if save_this_rank:
                    current_export_path = eval_frame_export_path
                    if all_rank_preview and dist_on:
