...
def save_ctsd_eval_frames_for_preview(
    ...
    allow_all_rank_export = bool(
        inference_config.get("all_rank_preview", False)
    )
    if (not allow_all_rank_export) and (not _preview_eval_rank0()):
        return
        ...
        for b in range(batch_size):
            ...
            if (
                ...
                if len(box_records_batch) == batch_size:
                    sample_box_records = box_records_batch[b]
                elif batch_size == 1:
                    sample_box_records = box_records_batch
            ...
            for t in range(sequence_length):
                ...
                if (
                    ...
                    and t < len(sample_box_records)
                ):
                    frame_record["boxes_3d"] = (
                        sample_box_records[t]
                    )
                    frame_record["box_format"] = str(
                        inference_config.get(
                ...
                for v in range(view_count):
                    ...
                    if (
                        export_box_params
                        and lidar_to_camera_key in batch
                        and "camera_intrinsics" in batch
                    ):
                        lidar_to_camera = (
                            batch[lidar_to_camera_key][b, t, v]
                            .detach()
                            .cpu()
                            .float()
                        )
                        ...
                        if tuple(
                            lidar_to_camera.shape[-2:]
                        ) != (4, 4):
                            raise ValueError(
                                "lidar_to_camera must be 4x4, "
                                f"got {tuple(lidar_to_camera.shape)}"
                            )

                        if camera_intrinsic.shape[-2] < 3:
                            raise ValueError(
                                "camera_intrinsics must contain "
                                "at least 3 rows"
                            )

                        K3 = camera_intrinsic[:3, :3]

                        lidar_to_image = torch.eye(
                            4,
                            dtype=torch.float32,
                        )
                        lidar_to_image[:3, :4] = (
                        ...
                        view_record[
                            "T_lidar_to_camera"
                        ] = lidar_to_camera.tolist()

                        view_record[
                            "lidar_to_image"
                        ] = lidar_to_image.tolist()
