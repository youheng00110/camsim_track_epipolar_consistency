...
def _lidar2camera_from_caminfo(cam_info):
    """Return camera_from_lidar as a 4x4 rigid transform.

    NuPlan gt_boxes are stored in the LiDAR coordinate frame.
    cam_info stores camera_from_lidar inversely as
...
class MotionDataset(torch.utils.data.Dataset):
    ...
    def get_instance_flow_images(
        ...
        allowed_categories = {
            str(name).lower()
            for name in self.instance_flow_image_settings.get(
                "categories",
        ...
        for time_index, info in enumerate(seq):
            ...
            if len(boxes) > 0 and len(tracks) == 0:
                raise KeyError(
                    "NuPlan instance flow requires track_token or gt_track_token."
                )

            records = []
            for box_index, box in enumerate(boxes):
                category_name = (
                    str(names[box_index]).lower()
                    if box_index < len(names)
                    else ""
                )
                ...
                if box_index >= len(tracks) or tracks[box_index] is None:
                    continue
                track_id = str(tracks[box_index])
                if not track_id:
                    continue
                box7 = np.asarray(box[:7], dtype=np.float32).reshape(-1)
                if box7.shape[0] != 7 or not np.isfinite(box7).all():
                    continue
                corners_frame = self._mdtoken_box_corners_lidar_xyz(box7)
            ...
            specs = []
            for view_index, cam_info in enumerate(cam_infos[time_index]):
                if cam_info is None:
                    raise KeyError(
                        "Missing NuPlan camera calibration while building instance flow."
                    )
                specs.append(
                    {
                        "intrinsic": np.asarray(
                            cam_Ks[time_index][view_index],
                            dtype=np.float32,
                        ).reshape(3, 3),
                        "camera_from_frame": _lidar2camera_from_caminfo(
                            cam_info
                        ),
                        "image_width": int(img_sizes[time_index][view_index][0]),
                        "image_height": int(img_sizes[time_index][view_index][1]),
                    }
                )
    ...
    def _mdtoken_dynamic_class_id(self, category_name):
        if hasattr(self, "get_layout_token_class_id"):
            try:
                return int(self.get_layout_token_class_id(category_name))
            except Exception:
                pass
    ...
    def _mdtoken_order_bev_polygon(self, pts):
    ...
    def _mdtoken_box_corners_lidar_xyz(self, box7):
        box7 = np.asarray(box7[:7], dtype=np.float32)

        corner_fn = globals().get("_box_corners_lidar_xyz", None)
        if corner_fn is not None:
            try:
                return corner_fn(box7, z_is_center=True)
            except TypeError:
                return corner_fn(box7)
    ...
    def _get_dynamic_object_bev_masks(
        self,
        info,
        bev_h,
        bev_w,
        ...
        for i, box in enumerate(boxes):
            ...
            if not np.isfinite(box_np).all():
                continue

            corners = self._mdtoken_box_corners_lidar_xyz(box_np)

            class_name = names[i] if i < len(names) else "car"
            cls_id = int(self._mdtoken_dynamic_class_id(class_name))
            ...
            poly = self._mdtoken_order_bev_polygon(uv)
    ...
    def _get_hdmap_bev_image(self, info):
        ...
        if bool(settings.get("append_dynamic_object_mask", False)):
            num_obj_classes = int(settings.get("num_object_classes", 10))
            obj_masks = self._get_dynamic_object_bev_masks(
                info,
                bev_h,
                bev_w,
                bev_from_ego,
    ...
    def _get_hdmap_image(self, info, cam_ch, im_size, cam_intrinsic_3x3):
        ...
        if cam_info is None:
            raise KeyError(f"Missing NuPlan camera calibration: {cam_ch}")
    ...
    # 3dbox from gt_line
    # ---------------------------
    def _get_3dbox_records_from_gtline(self, info):
        """Return independent 7-DoF GT boxes for one frame.

        box7_lidar order:
            [x, y, z, dx, dy, dz, yaw]

        x, y, z:
            box center in LiDAR coordinates

        dx, dy, dz:
            box extent along the local x/y/z axes

        yaw:
            rotation around the LiDAR z axis, in radians
        """
        boxes = info.get("gt_boxes", None)
        if boxes is None:
            boxes = []
        ...
        for box_index, box in enumerate(boxes):
            ...
            track_token = (
                str(tracks[box_index])
                if (
                    box_index < len(tracks)
                    and tracks[box_index] is not None
                )
                else ""
            )
    ...
    def _get_3dbox_image_from_gtline(self, info, im_size, lidar2image_4x4):
        ...
        return img
    #################################
    def _layout_box_visible_in_nuplan_camera(
        self,
        corners_np,
        cam_info,
        im_size,
        ...
        if cam_info is None:
            raise KeyError("Missing NuPlan camera calibration for box visibility.")
        ...
        image_w = int(im_size[0])
        image_h = int(im_size[1])

        if image_w <= 0 or image_h <= 0:
            return False

        K3 = np.asarray(cam_intrinsic_3x3, dtype=np.float32).reshape(3, 3)
        lidar2image = _lidar2image_from_caminfo(cam_info, K3)

        uvz = _project_pts(corners_np, lidar2image)
        depth = uvz[:, 2]
        ...
        pts2 = uvz[valid_depth, :2]

        if pts2[:, 0].max() < 0.0:
            return False
        if pts2[:, 0].min() >= float(image_w):
            return False
        if pts2[:, 1].max() < 0.0:
            return False
        if pts2[:, 1].min() >= float(image_h):
            return False
    ...
    def get_layout_token_boxes(
        ...
        if cam_infos is None or img_sizes is None or cam_Ks is None:
            raise ValueError(
                "NuPlan layout tokens require camera info, image sizes, and intrinsics."
            )

        cam_infos = list(cam_infos)
        if len(cam_infos) == 0:
            raise ValueError("NuPlan layout tokens require at least one camera.")
        if len(img_sizes) != len(cam_infos) or len(cam_Ks) != len(cam_infos):
            raise ValueError("NuPlan camera metadata length mismatch.")
        view_count = len(cam_infos)
        ...
        for i, box in enumerate(boxes):
            ...
            if not np.isfinite(box_corners).all():
                continue

            class_name = names[i] if i < len(names) else "car"
            class_id = int(self.get_layout_token_class_id(class_name))
            box_corners_t = torch.from_numpy(box_corners).float()

            for view_idx in range(view_count):
                corners[view_idx, kept] = box_corners_t
    ...
    def __getitem__(self, index):
        ...
        for x in seq:
            ...
            for cam_i, (cam_ch, p) in enumerate(zip(self.sensor_channels, paths)):
                im = _try_open_png(p)
                if im is None:
                    raise RuntimeError(f"Failed to read NuPlan camera image: {p}")
        ...
        for row_infos in cam_infos:
            ...
            for cam_info in row_infos:
                if cam_info is None:
                    raise KeyError("Missing NuPlan camera calibration.")
                row_transforms.append(
                    _lidar2camera_from_caminfo(cam_info)
                )

            lidar_to_camera.append(row_transforms)

        result["lidar_to_camera"] = torch.tensor(
            np.asarray(lidar_to_camera),
            dtype=torch.float32,
        )
            ...
            for t, info in enumerate(seq):
                bbox_corners, bbox_classes, bbox_masks = \
                    self.get_layout_token_boxes(
                        info,
                        cam_infos=cam_infos[t],
                        img_sizes=img_sizes[t],
                        cam_Ks=cam_Ks[t],
                    )
        ...
        # ---- 3dbox_images (gt_line), no disk cache
        imgs = []
        ...
        # ---- hdmap_images (nuplan), no disk cache
        imgs = []
        ...
        # ---- hdmap_bev_images, no disk cache
