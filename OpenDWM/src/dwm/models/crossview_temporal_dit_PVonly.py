...
class DiTCrossviewTemporalConditionModel(diffusers.SD3Transformer2DModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        elif perspective_modeling_type == "pvonly":
            self.view_id_embedding = torch.nn.Embedding(
                max_view_count,
                self.inner_dim,
            )
            self.view_id_mlp = torch.nn.Sequential(
                torch.nn.Linear(self.inner_dim, self.inner_dim * 4),
                torch.nn.SiLU(),
                torch.nn.Linear(self.inner_dim * 4, self.inner_dim),
            )
    ...
    def forward(
        ...
        if should_add_dim:
            ...
            if camera_ids is not None:
                camera_ids = camera_ids.unsqueeze(2)    
        ...
        elif self.perspective_modeling_type == "pvonly":
            if camera_ids is None:
                camera_ids = torch.arange(
                    view_count,
                    device=hidden_states.device,
                ).view(1, 1, view_count).expand(
                    batch_size,
                    sequence_length,
                    view_count,
                )

            camera_ids = camera_ids.to(
                device=hidden_states.device,
                dtype=torch.long,
            )

            if int(camera_ids.max()) >= self.max_view_count:
                raise ValueError(
                    f"pvonly camera id exceeds max_view_count="
                    f"{self.max_view_count}, got max id={int(camera_ids.max())}"
                )

            view_cam_emb = self.view_id_embedding(
                camera_ids.reshape(-1)
            )
            view_cam_emb = self.view_id_mlp(
                view_cam_emb
            ).to(dtype=hidden_states.dtype).unsqueeze(1)

            encoder_hidden_states = torch.cat(
                [encoder_hidden_states, view_cam_emb],
                dim=1,
            )
