...
from dwm.models.rayrope_rowwise_block import VTRayRoPERowwiseAttentionBlock
...
class DiTCrossviewTemporalConditionModel(diffusers.SD3Transformer2DModel):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if enable_crossview:
            ...
            if perspective_modeling_type == "rayrope" and crossview_attention_type == "rowwise":
                self.crossview_transformer_blocks = torch.nn.ModuleList([
                    VTRayRoPERowwiseAttentionBlock(
                        inner_dim,
                        inner_dim,
                        num_attention_heads,
                        attention_head_dim,
                        qk_norm=qk_norm_on_additional_modules,
                        rayrope_config=self.rayrope_config,
                    )
                    for _ in range(len(crossview_block_layers))
                ])
            else:
    ...
    def forward_crossview_block_and_mix_result(
        ...
        if self.crossview_attention_type == "fuse":
            ...
            if self.perspective_modeling_type == "rayrope":
                ...
                K_token[..., 1, 1] = K_token[..., 1, 1] * height
                K_token[..., 0, 2] = K_token[..., 0, 2] * width
                K_token[..., 1, 2] = K_token[..., 1, 2] * height

                w2cs = torch.linalg.inv(
                    camera2referego.to(dtype=torch.float32)
                ...
                w2cs = w2cs.reshape(batch_size * sequence_length, view_count, 4, 4)
                K_token = K_token.reshape(batch_size * sequence_length, view_count, 3, 3)

                w2cs = w2cs.repeat_interleave(height, dim=0)
                K_token = K_token.repeat_interleave(height, dim=0)
    ...
    def forward(
        ...
        elif self.perspective_modeling_type == "rayrope":
            view_cam_emb = 0
