...
class DiTCrossviewTemporalConditionModel(diffusers.SD3Transformer2DModel):
    ...
    def forward(
        ...
        if self.perspective_modeling_type == "implicit":
            ...
            if camera_intrinsics_norm is None or camera_transforms is None:
                raise ValueError(
                    "PETR camera encoding requires camera_intrinsics_norm and camera_transforms. "
            ...
            # camera_transforms is camera -> current-frame ego.
            # PETR therefore uses the same current-ego coordinate system
            # as the per-frame BEV map and bbox conditions.
            if camera_transforms.ndim != 5:
                raise ValueError(
                    "camera_transforms should be [B,T,V,4,4] for "
                    "current-ego PETR, but got "
                    f"{tuple(camera_transforms.shape)}."
                )

            camera_for_petr = camera_transforms

            if not hasattr(self, "_petr_current_ego_debug_printed"):
                self._petr_current_ego_debug_printed = False

            if not self._petr_current_ego_debug_printed:
                print(
                    "[PETR geometry] use camera_transforms: "
