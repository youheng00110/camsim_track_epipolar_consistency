...
class PluckerEncoder(torch.nn.Module):
    ...
    def forward(self, rays_d, rays_m):
        batchsize, height, width, _ = rays_d.shape
        ...
        x = x.view(batchsize, height, width, x.shape[-1])
...
def save_plucker_debug_visuals(
    save_root,
    batch_size,
    sequence_length,
    view_count,
    ...
    rays_o = rays_o.detach().float().cpu().reshape(
        batch_size, sequence_length, view_count, 3
    )
    rays_d = rays_d.detach().float().cpu().reshape(
        batch_size, sequence_length, view_count, height, width, 3
    )
    rays_m = rays_m.detach().float().cpu().reshape(
        batch_size, sequence_length, view_count, height, width, 3
    )
    raymap = raymap.detach().float().cpu().reshape(
        batch_size, sequence_length, view_count, height, width, -1
    )
    view_cam_emb = view_cam_emb.detach().float().cpu().reshape(
        batch_size, sequence_length, view_count, height, width, -1
    )
    ...
    dir_rgb = ((rays_d[b, t].permute(0, 3, 1, 2) + 1.0) * 0.5).clamp(0, 1)
    torchvision.utils.save_image(
        dir_rgb,
        os.path.join(save_root, "plucker_dir_rgb_b0_t0.png"),
        nrow=view_count,
    )

    m_norm = torch.linalg.norm(rays_m[b, t], dim=-1).unsqueeze(1)
    m_min = m_norm.amin(dim=(-2, -1), keepdim=True)
    m_max = m_norm.amax(dim=(-2, -1), keepdim=True)
    m_norm = (m_norm - m_min) / (m_max - m_min + 1e-6)
    torchvision.utils.save_image(
        m_norm,
        os.path.join(save_root, "plucker_m_norm_b0_t0.png"),
        nrow=view_count,
    )

    orth = torch.abs((rays_d[b, t] * rays_m[b, t]).sum(dim=-1)).unsqueeze(1)
    orth_max = orth.amax(dim=(-2, -1), keepdim=True)
    orth = orth / (orth_max + 1e-12)
    torchvision.utils.save_image(
        orth,
        os.path.join(save_root, "plucker_orth_b0_t0.png"),
        nrow=view_count,
    )

    raymap_norm = torch.linalg.norm(raymap[b, t], dim=-1).unsqueeze(1)
    rm_min = raymap_norm.amin(dim=(-2, -1), keepdim=True)
    rm_max = raymap_norm.amax(dim=(-2, -1), keepdim=True)
    raymap_norm = (raymap_norm - rm_min) / (rm_max - rm_min + 1e-6)
    torchvision.utils.save_image(
        raymap_norm,
        os.path.join(save_root, "plucker_raymap_norm_b0_t0.png"),
        nrow=view_count,
    )

    emb_norm = torch.linalg.norm(view_cam_emb[b, t], dim=-1).unsqueeze(1)
    em_min = emb_norm.amin(dim=(-2, -1), keepdim=True)
    em_max = emb_norm.amax(dim=(-2, -1), keepdim=True)
    emb_norm = (emb_norm - em_min) / (em_max - em_min + 1e-6)
    torchvision.utils.save_image(
        emb_norm,
        os.path.join(save_root, "plucker_view_cam_emb_norm_b0_t0.png"),
        nrow=view_count,
    )
    ...
    orth_all = torch.abs((rays_d * rays_m).sum(dim=-1))
    orth_mean = orth_all.mean().item()
    orth_max_all = orth_all.max().item()

    with open(
        os.path.join(save_root, "plucker_stats_b0_t0.txt"),
        "w",
        encoding="utf-8",
    ) as f:
        f.write(f"patch_grid_height={height}\n")
        f.write(f"patch_grid_width={width}\n")
        f.write(f"orth_mean={orth_mean:.8e}\n")
        f.write(f"orth_max={orth_max_all:.8e}\n")
        f.write("\n")

        for v in range(view_count):
            f.write(f"===== view {v} =====\n")
            f.write(f"rays_o = {rays_o[b, t, v].tolist()}\n")
            f.write(
                f"center_rays_d = {rays_d[b, t, v, center_h, center_w].tolist()}\n"
            )
            f.write(
                f"center_rays_m = {rays_m[b, t, v, center_h, center_w].tolist()}\n"
            )
            f.write(
                f"center_raymap_norm = "
                f"{torch.linalg.norm(raymap[b, t, v, center_h, center_w]).item():.8f}\n"
            )
            f.write(
                f"center_view_cam_emb_norm = "
                f"{torch.linalg.norm(view_cam_emb[b, t, v, center_h, center_w]).item():.8f}\n"
            )
            f.write("\n")
...
def build_pv_boxaware_mask(
    ...
    """
    Build a latent-grid box-aware mask from the original PV condition images.

    Args:
        condition_image_tensor: [B, T, V, C, H_img, W_img]. In the current
            PV condition pipeline, the first 3 channels are usually 3dbox_images
            and the next 3 channels are hdmap_images.
        height, width: latent patch grid size after pos_embed, not image size.
        channel_start/channel_count: channel slice used as the PV box source.
        threshold: non-zero threshold for box-line pixels.
        dilate_pixels: max-pool dilation on the PV mask before downsampling so
            thin projected box lines survive latent-grid pooling.
        detach: detach PV condition before building the mask. Recommended since
            this mask is a routing/weighting signal, not a learned image branch.

    Returns:
        boxaware_mask: [B*T*V, H*W], values in [0, 1].
    """
    if condition_image_tensor is None:
        return None

    if condition_image_tensor.ndim != 6:
        raise ValueError(
            "PV box-aware expects condition_image_tensor [B,T,V,C,H,W], "
            f"but got {tuple(condition_image_tensor.shape)}."
        )

    batch_size, sequence_length, view_count, channel_count_total = \
        condition_image_tensor.shape[:4]

    channel_start = int(channel_start)
    channel_count = int(channel_count)
    channel_end = channel_start + channel_count

    if channel_start < 0 or channel_count <= 0 or channel_end > channel_count_total:
        raise ValueError(
            "Invalid PV box-aware channel slice: "
            f"start={channel_start}, count={channel_count}, "
            f"total_channels={channel_count_total}."
        )

    box_source = condition_image_tensor[:, :, :, channel_start:channel_end]
    if detach:
        box_source = box_source.detach()

    out_dtype = condition_image_tensor.dtype
    box_score = box_source.float().abs().amax(dim=3)
    flat_mask = (box_score > float(threshold)).to(dtype=torch.float32)
    flat_mask = flat_mask.reshape(
    ...
    if dilate_pixels > 1:
        if dilate_pixels % 2 == 0:
            dilate_pixels += 1
        flat_mask = F.max_pool2d(
            flat_mask,
            kernel_size=dilate_pixels,
            stride=1,
            padding=dilate_pixels // 2,
        )
...
class DiTCrossviewTemporalConditionModel(diffusers.SD3Transformer2DModel):
    ...
    def __init__(
        ...
        enable_boxaware: bool = False,
        boxaware_mode: str = "scale",
        boxaware_apply_to: str = "crossview",
        boxaware_init_scale: float = 0.25,
        boxaware_pv_channel_start: int = 0,
        boxaware_pv_channel_count: int = 3,
        boxaware_pv_threshold: float = 0.01,
        boxaware_pv_dilate_pixels: int = 5,
        boxaware_pv_detach: bool = True,
        boxaware_debug: bool = False,
        mask_module=None,
        **kwargs
    ):
        super().__init__(
        ...
        self.enable_boxaware = bool(enable_boxaware)
        self.boxaware_mode = str(boxaware_mode)
        self.boxaware_apply_to = str(boxaware_apply_to)
        self.boxaware_pv_channel_start = int(boxaware_pv_channel_start)
        self.boxaware_pv_channel_count = int(boxaware_pv_channel_count)
        self.boxaware_pv_threshold = float(boxaware_pv_threshold)
        self.boxaware_pv_dilate_pixels = int(boxaware_pv_dilate_pixels)
        self.boxaware_pv_detach = bool(boxaware_pv_detach)
        self.boxaware_debug = bool(boxaware_debug)
        ...
        if self.enable_boxaware:
            self.boxaware_scale = torch.nn.Parameter(
                torch.tensor([float(boxaware_init_scale)], dtype=torch.float32)
            )
            self.boxaware_token_bias = torch.nn.Parameter(
                torch.zeros(1, 1, inner_dim, dtype=torch.float32)
            )
        else:
            self.boxaware_scale = None
            self.boxaware_token_bias = None
    ...
    def apply_boxaware_weight(
        self,
        hidden_states: torch.Tensor,
        boxaware_mask: torch.Tensor,
    ):
        if boxaware_mask is None or not self.enable_boxaware:
            return hidden_states

        boxaware_mask = boxaware_mask.to(
            device=hidden_states.device,
            dtype=hidden_states.dtype,
        ).unsqueeze(-1)

        if self.boxaware_mode in ["scale", "scale_bias"]:
            scale = self.boxaware_scale.to(
                device=hidden_states.device,
                dtype=hidden_states.dtype,
            )
            hidden_states = hidden_states * (1.0 + scale * boxaware_mask)

        if self.boxaware_mode in ["bias", "scale_bias"]:
            bias = self.boxaware_token_bias.to(
                device=hidden_states.device,
                dtype=hidden_states.dtype,
            )
            hidden_states = hidden_states + boxaware_mask * bias
    ...
    def forward_crossview_block_and_mix_result(
        ...
        crossview_attention_mask, crossview_attention_index, boxaware_mask=None
    ):
        crossview_hidden_states = hidden_states + \
            view_emb      # [b*T*V, h*w, c]
        if self.boxaware_apply_to in ["crossview", "all"]:
            crossview_hidden_states = self.apply_boxaware_weight(
                crossview_hidden_states, boxaware_mask
            )
    ...
    def forward_temporal_block_and_mix_result(
        ...
        if self.boxaware_apply_to in ["temporal", "all"]:
            temporal_hidden_states = self.apply_boxaware_weight(
                temporal_hidden_states, boxaware_mask
            )
    ...
    def forward(
        ...
        view_cam_emb = 0
        camera_intrinsics_grid = None
        camera_for_boxaware = None
        if self.perspective_modeling_type == "implicit":
            ...
            if camera_intrinsics_norm is None:
                raise ValueError(
                    "camera_intrinsics_norm is required for explicit Plucker."
                )
            camera_for_boxaware = camera2referego
            if camera_for_boxaware is None:
                camera_for_boxaware = camera_transforms
            if camera_for_boxaware is None:
                raise ValueError(
                    "camera2referego or camera_transforms is required for explicit Plucker."
                )

            camera_intrinsics_grid = camera_intrinsics_norm.to(
                device=hidden_states.device,
                dtype=hidden_states.dtype,
            ).clone()
            camera_intrinsics_grid[..., 0, 0] = \
                camera_intrinsics_grid[..., 0, 0] * width
            camera_intrinsics_grid[..., 1, 1] = \
                camera_intrinsics_grid[..., 1, 1] * height
            camera_intrinsics_grid[..., 0, 2] = \
                camera_intrinsics_grid[..., 0, 2] * width
            camera_intrinsics_grid[..., 1, 2] = \
                camera_intrinsics_grid[..., 1, 2] * height

            camera_for_boxaware = camera_for_boxaware.to(
                device=hidden_states.device,
                dtype=hidden_states.dtype,
            )

            rays_o, rays_d = get_rays(
                camera_intrinsics_grid.flatten(0, 2),
                camera_for_boxaware.flatten(0, 2),
                (height, width)
            )
        ...
        boxaware_mask = None
        if self.enable_boxaware:
            boxaware_mask = build_pv_boxaware_mask(
                condition_image_tensor,
                height,
                width,
                channel_start=self.boxaware_pv_channel_start,
                channel_count=self.boxaware_pv_channel_count,
                threshold=self.boxaware_pv_threshold,
                dilate_pixels=self.boxaware_pv_dilate_pixels,
                detach=self.boxaware_pv_detach,
            )

            if self.boxaware_debug and not hasattr(self, "_boxaware_debug_printed"):
                if boxaware_mask is None:
                    print("[boxaware-pv] condition_image_tensor is None; mask disabled.")
                else:
                    print("[boxaware-pv] mask shape:", tuple(boxaware_mask.shape))
                    print("[boxaware-pv] mask mean/max/scale:",
                          float(boxaware_mask.float().mean()),
                          float(boxaware_mask.float().max()),
                          float(self.boxaware_scale.detach().float())
                          if self.boxaware_scale is not None else 0.0)
                    print("[boxaware-pv] channel_start/count/threshold/dilate:",
                          self.boxaware_pv_channel_start,
                          self.boxaware_pv_channel_count,
                          self.boxaware_pv_threshold,
                          self.boxaware_pv_dilate_pixels)
                self._boxaware_debug_printed = True
            ...
            if self.enable_temporal and i in self.temporal_block_layers:
                ...
                if self.training and self.temporal_gradient_checkpointing:
                    hidden_states = torch.utils.checkpoint.checkpoint(
                        ...
                        disable_temporal, boxaware_mask, use_reentrant=False)
                else:
                    hidden_states = self.forward_temporal_block_and_mix_result(
                        ...
                        hidden_states, sequence_emb,
                        batch_size, sequence_length, view_count, width,
                        disable_temporal, boxaware_mask)
            ...
            if self.enable_crossview and i in self.crossview_block_layers:
                ...
                if self.training and self.crossview_gradient_checkpointing:
                    ...
                    hidden_states = self.forward_crossview_block_and_mix_result(
                        ...
                        crossview_attention_index, boxaware_mask)
