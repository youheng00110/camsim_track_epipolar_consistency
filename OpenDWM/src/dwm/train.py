...
def resolve_dataset_shift_stage(config, global_step):
    dataset_shift = config.get("dataset_shift")
    if dataset_shift is None or not dataset_shift.get("enabled", False):
        return None

    stages = dataset_shift.get("stages", [])
    if len(stages) == 0:
        raise ValueError("dataset_shift.enabled is true, but no stages are configured")

    active_stage = None
    previous_start_step = -1
    for stage in stages:
        start_step = int(stage["start_step"])
        if start_step <= previous_start_step:
            raise ValueError("dataset_shift.stages must be strictly ordered by start_step")
    ...
    if active_stage is None:
        raise ValueError("the first dataset_shift stage must start at step 0")

    return active_stage


def create_training_dataset(config, stage=None):
    dataset_config = copy.deepcopy(config["training_dataset"])

    if stage is not None:
        base_dataset_config = dataset_config.get("base_dataset", {})
        all_dataset_configs = base_dataset_config.get("datasets")
        if all_dataset_configs is None:
            raise ValueError(
                "dataset_shift requires training_dataset.base_dataset.datasets")

        dataset_indices = stage.get("dataset_indices", [])
        if len(dataset_indices) == 0:
            raise ValueError(
                "dataset_shift stage '{}' has no dataset_indices".format(
                    stage.get("name", "unnamed")
                )
            )

        invalid_indices = [
            i for i in dataset_indices
            if i < 0 or i >= len(all_dataset_configs)
        ]
        if len(invalid_indices) > 0:
            raise IndexError(
                "dataset_shift stage '{}' has invalid dataset_indices {} for {} "
                "configured datasets".format(
                    stage.get("name", "unnamed"),
                    invalid_indices,
                    len(all_dataset_configs),
                )
            )

        base_dataset_config["datasets"] = [
            all_dataset_configs[i] for i in dataset_indices
        ]

    return dwm.common.create_instance_from_config(dataset_config)


def create_training_dataloader(config, training_dataset, ddp):
    ...
    if not ddp:
        ...
        return None, training_dataloader
    ...
    return training_datasampler, training_dataloader
...
if __name__ == "__main__":
    ...
    dataset_shift = config.get("dataset_shift", {})
    dataset_shift_enabled = dataset_shift.get("enabled", False)
    current_training_stage = resolve_dataset_shift_stage(config, global_step)

    training_dataset = create_training_dataset(config, current_training_stage)
    training_datasampler, training_dataloader = create_training_dataloader(
        config, training_dataset, ddp
    )
    if dataset_shift_enabled and should_log:
        print(
            "[dataset-shift] initial step={} -> stage='{}', "
            "dataset_indices={}, items={}".format(
                global_step,
                current_training_stage["name"],
                current_training_stage["dataset_indices"],
    ...
    # train loop
    if dataset_shift_enabled:
        max_steps = int(dataset_shift["max_steps"])
        extra_checkpoint_steps = set(
            int(step) for step in dataset_shift.get("checkpoint_steps", [])
        )
    else:
        max_steps = None
        extra_checkpoint_steps = set()
    ...
    while True:
        if dataset_shift_enabled:
            ...
            next_training_stage = resolve_dataset_shift_stage(config, global_step)
            if next_training_stage["name"] != current_training_stage["name"]:
                if torch.distributed.is_initialized():
                    torch.distributed.barrier()

                current_training_stage = next_training_stage
                training_dataset = create_training_dataset(
                    config, current_training_stage
                )
                training_datasampler, training_dataloader = \
                    create_training_dataloader(config, training_dataset, ddp)

                if should_log:
                    print(
                        "[dataset-shift] step={} -> stage='{}', "
                        "dataset_indices={}, items={}".format(
                            global_step,
                            current_training_stage["name"],
                            current_training_stage["dataset_indices"],
                ...
                if torch.distributed.is_initialized():
                    torch.distributed.barrier()
        elif epoch >= config["train_epochs"]:
            break
        ...
        if should_log:
            if dataset_shift_enabled:
                loader_desc = "Stage {} / Epoch {}".format(
                    current_training_stage["name"], epoch
                )
            else:
                loader_desc = "Epoch {}".format(epoch)
            loader = tqdm(
                training_dataloader,
        ...
        for batch_idx, batch in enumerate(loader, start=1):
            ...
            regular_checkpoint = (
                args.checkpointing_steps > 0 and
                global_step % args.checkpointing_steps == 0
            )
            if regular_checkpoint or global_step in extra_checkpoint_steps:
            ...
            if dataset_shift_enabled:
                ...
                next_training_stage = resolve_dataset_shift_stage(
                    config, global_step
                )
                if next_training_stage["name"] != current_training_stage["name"]:
                    if should_log:
                        print(
                            "[dataset-shift] reached transition at step {}: "
                            "'{}' -> '{}'".format(
                                global_step,
                                current_training_stage["name"],
                                next_training_stage["name"],
                            )
                        )
                    break

        if should_log and completed_batches > 0:
            epoch_total_time = time.time() - epoch_start_time
            print(
                "Epoch {} done. total_time={:.1f} min, avg_step={:.2f} s".format(
                    epoch,
                    epoch_total_time / 60.0,
                    epoch_total_time / completed_batches
                )
            )
