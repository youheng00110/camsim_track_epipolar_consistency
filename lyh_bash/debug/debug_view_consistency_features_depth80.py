...
def build_depth_limited_target_mask(
    ...
    """Triangulate source-ray range for every target patch center.

    The source query patch defines one ray in camera A. Each target patch
    center defines one ray in camera B. For each target ray, solve the
    least-squares two-ray system

        R * (lambda_a * r_a) + t ~= lambda_b * r_b

    where camera_b_from_camera_a = [R|t].

    lambda_a is the metric distance along the normalized source camera ray.
    A target patch is depth-valid when:
      - lambda_a is inside [min_depth, max_depth]
      - lambda_b > 0, so the reconstructed point is in front of camera B
      - the two-ray solve is not degenerate

    Epipolar-band filtering is still applied separately by the caller.
    """
    if max_depth <= min_depth:
        raise ValueError(
            f"max_depth must be > min_depth, got "
            f"{min_depth}..{max_depth}."
        )

    ka = vc.scale_normalized_intrinsics_to_feature_grid(
        camera_intrinsics_a_norm.float().cpu(),
        feature_height,
        feature_width,
    )
    kb = vc.scale_normalized_intrinsics_to_feature_grid(
        camera_intrinsics_b_norm.float().cpu(),
        feature_height,
        feature_width,
    )

    query_coordinate = query_coordinate.float().cpu()
    target_coordinates = target_coordinates.float().cpu()
    ...
    ray_b = torch.linalg.solve(
        kb,
        target_coordinates.transpose(0, 1),
    ).transpose(0, 1)
    ray_b = torch.nn.functional.normalize(
        ray_b,
        dim=1,
    )

    camera_b_from_camera_a = (
        torch.linalg.inv(camera2referego_b.float().cpu())
        @ camera2referego_a.float().cpu()
    )
    rotation = camera_b_from_camera_a[:3, :3]
    translation = camera_b_from_camera_a[:3, 3]

    source_ray_in_b = rotation @ ray_a
    source_ray_in_b = torch.nn.functional.normalize(
        source_ray_in_b,
        dim=0,
    )

    # Solve [u, -v] [lambda_a, lambda_b]^T = -t.
    # With normalized u/v, the 2x2 normal equations have a compact form.
    dot_uv = torch.sum(
        ray_b * source_ray_in_b.unsqueeze(0),
        dim=1,
    )
    determinant = 1.0 - dot_uv.square()

    rhs_source = -torch.dot(
        source_ray_in_b,
        translation,
    )
    rhs_target = torch.sum(
        ray_b * translation.unsqueeze(0),
        dim=1,
    )

    safe_det = determinant.clamp_min(1e-8)
    source_depth = (
        rhs_source + dot_uv * rhs_target
    ) / safe_det
    target_depth = (
        dot_uv * rhs_source + rhs_target
    ) / safe_det

    valid = (
        determinant.gt(1e-6)
        & source_depth.ge(float(min_depth))
        & source_depth.le(float(max_depth))
        & target_depth.gt(0.0)
        & torch.isfinite(source_depth)
        & torch.isfinite(target_depth)
    )
    return valid, source_depth, target_depth
...
def main():
    ...
    if args.max_depth <= args.min_depth:
        raise ValueError(
            f"--max-depth must be greater than --min-depth, got "
    ...
    for pair_index in range(max_pairs):
        ...
        (
            ...
            source_depths,
            target_depths,
        ) = build_depth_limited_target_mask(
        ...
        labels = (
            ...
            (
                f"B depth-limited epipolar band "
                f"[{args.min_depth:g},{args.max_depth:g}]m "
                f"+ raw-feature Top-{len(top_indices)}"
            ),
        )
        ...
        filename = (
            ...
            f"t{time_b:02d}_v{view_b}_raw1536_depth0_80m.png"
        )
        ...
        record = {
            ...
            "full_epipolar_positive_patch_count": full_positive_count,
            "depth_limited_positive_patch_count": len(positive_indices),
        ...
        print(
            f"        query={query_index} fg={query_is_foreground} "
            f"full_positive={full_positive_count} "
            f"depth_{args.min_depth:g}_{args.max_depth:g}m="
    ...
    metadata = {
        ...
        "note": (
            ...
            "The yellow positive band is additionally filtered by "
            "two-ray triangulated source range in [min_depth_m,max_depth_m]."
        ),
    }
